...
async function preCompileDir(dir: string) {
	...
	await Promise.all(
		files.map((filename) => {
			...
			if (isDemoDir(filePath) || isTestDir(filePath)) return fe.remove(filePath)
			if (isDir(filePath)) return preCompileDir(filePath)
			if (isSfc(filePath)) return compileSfc(filePath)
...
async function compileFile(filePath: string, format: Format) {
	if (isScript(filePath)) return compileScript(filePath, format)
	if (isStyle(filePath)) return compileStyle(filePath)
	if (isAsset(filePath)) return Promise.resolve()
...
async function buildTypeDeclarations() {
	...
	if (fe.existsSync(tsConfig)) await execa('tsc', ['-p', tsConfig], { stdout: 'inherit', stderr: 'inherit' })
}
