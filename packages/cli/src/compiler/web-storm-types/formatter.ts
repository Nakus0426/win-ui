...
import type { Articles } from './parser.js'
import type { VueEventArgument, VueTag } from './types.js'

function formatComponentName(name: string, tagPrefix: string) {
	return tagPrefix + kebabCase(name)
}

function getNameFromTableTitle(tableTitle: string, tagPrefix: string) {
	tableTitle = tableTitle.trim()
	if (tableTitle.includes(' ')) return formatComponentName(tableTitle, tagPrefix).split(' ')[0]
}

function findTag(vueTags: VueTag[], name: string) {
	const matched = vueTags.find((item) => item.name === name)
	if (matched) return matched
	const newTag: VueTag = { name }
	vueTags.push(newTag)
	return newTag
}

function removeVersion(str: string) {
...
export function formatter(vueTags: VueTag[], articles: Articles, tagPrefix = '') {
	...
	tables.forEach((item) => {
		const { table } = item
		const prevArticle = articles[articles.indexOf(item) - 1]
		if (!prevArticle || !prevArticle.content || !table || !table.body) return
